...
exports.getSchedulesByStaffId = async (req, res) => {
  try {
    const staffId = req.params.staffId;
    const schedules = await Schedule.find({ staffId }).populate('staffId', 'name email phone role staffId');
    res.status(200).json(schedules);
  } catch (error) {
    console.log(error);
    res.status(400).json({ message: 'Error fetching schedules for staff member' });
  }
};
